package com.github.auri_f5bde6.xeno_early_start.mixins.block;
...
public class BlastFurnaceBlockMixin extends AbstractBlockMixin {
    @Unique
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random, Operation<Void> original) {
        super.scheduledTick(state, world, pos, random, original);
        if (state.get(BlastFurnaceBlock.LIT) && XenoEarlyStartConfig.config.blockChanges.getBlastFurnaceSetNearbyBlockOnFire()) {
    ...
    public void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify, Operation<Void> original) {
        super.onBlockAdded(state, world, pos, oldState, notify, original);
